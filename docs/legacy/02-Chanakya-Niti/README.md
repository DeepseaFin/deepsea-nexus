# 02-Chanakya-Niti

## Purpose

Store strategic doctrine inspired by Chanakya Niti for long-term statecraft and institutional strategy.

## Scope

This section contains official records and reference documents that belong strictly to the Chanakya-Niti domain within the Deepsea Legacy Repository.

## Audience

Leadership, domain owners, custodians, auditors, and authorized contributors who require durable institutional context.

## Document Types

- Foundational reference notes
- Governance and policy records
- Controlled procedures and playbooks
- Historical decision or learning records
- Supporting annexures where applicable

## Ownership

Primary ownership: Assigned custodian for Chanakya-Niti.

Stewardship: Deepsea leadership and designated DNOS knowledge owners.
