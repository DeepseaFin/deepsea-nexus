...
export default function RiskWorkspace() {
  ...
  return (
    <div className="rounded-2xl border border-slate-800 bg-slate-900 p-6 shadow-xl sm:p-8">
      ...
      <WorkspaceTabs tabs={RISK_TABS} activeTab={activeTab} onTabChange={setActiveTab} />
        ...
        <div className="mt-6 rounded-xl border border-slate-800 bg-slate-950/80 p-4 sm:p-5">
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
            <div className="rounded-lg border border-slate-800 bg-slate-900/60 p-3">
              ...
              <p className="mt-1 text-sm font-semibold text-slate-100 capitalize">{evaluation.exposureAssessment.concentrationRisk}</p>
            </div>
            <div className="rounded-lg border border-slate-800 bg-slate-900/60 p-3">
          ...
          <div className="rounded-xl border border-slate-800 bg-slate-950/70 p-4">
            ...
            <div className="mt-3 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
              <div className="rounded-lg border border-slate-800 bg-slate-900/60 p-3">
                ...
                <p className="mt-1 text-sm font-semibold text-slate-100">{formatCurrency(evaluation.exposureAssessment.requestedFunding, { currency: evaluation.exposureAssessment.currency })}</p>
              </div>
              <div className="rounded-lg border border-slate-800 bg-slate-900/60 p-3">
                <p className="text-xs uppercase tracking-wide text-slate-500">Existing Exposure</p>
                <p className="mt-1 text-sm font-semibold text-slate-100">{formatCurrency(evaluation.exposureAssessment.existingExposure, { currency: evaluation.exposureAssessment.currency })}</p>
              </div>
              <div className="rounded-lg border border-slate-800 bg-slate-900/60 p-3">
                <p className="text-xs uppercase tracking-wide text-slate-500">Credit Limit</p>
                <p className="mt-1 text-sm font-semibold text-slate-100">{formatCurrency(evaluation.exposureAssessment.creditLimit, { currency: evaluation.exposureAssessment.currency })}</p>
              </div>
              <div className="rounded-lg border border-slate-800 bg-slate-900/60 p-3">
                <p className="text-xs uppercase tracking-wide text-slate-500">Projected Utilization</p>
                <p className="mt-1 text-sm font-semibold text-slate-100">{formatPercentage(evaluation.exposureAssessment.utilizationPercent, { maximumFractionDigits: 1 })}</p>
              </div>
            </div>
            <div className="mt-3 flex flex-wrap gap-2">
              {evaluation.exposureAssessment.notes.map((note, index) => (
                <span key={`exposure-note-${index + 1}`} className="rounded-full border border-slate-700 bg-slate-900 px-3 py-1 text-xs text-slate-300">{note}</span>
              ))}
            </div>
          </div>
        ) : null}
                ...
                {policy.sections.product.checks.map((checkResult) => (
                  <div key={checkResult.id} className="rounded-lg border border-slate-800 bg-slate-950/70 p-3 text-sm text-slate-300">
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      ...
                      <span className={`rounded-full border px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wide ${policyCheckStatusStyles[checkResult.status]}`}>{checkResult.status}</span>
                    </div>
