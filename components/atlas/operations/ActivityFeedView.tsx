...
export default function ActivityFeedView({ feed, className }: ActivityFeedViewProps) {
  return (
    <section
    ...
    >
      <header className="mb-5 flex flex-wrap items-start justify-between gap-3 border-b border-slate-800/80 pb-4">
        <div className="min-w-0">
          <p className="text-[11px] uppercase tracking-[0.2em] text-slate-500">Activity Feed</p>
          <h2 className="mt-1 text-xl font-semibold tracking-tight text-slate-100">Chronological Operational Activity</h2>
          <p className="mt-2 text-sm text-slate-400">Read-only presentation layer for operational activity events.</p>
        </div>
        <div className="flex items-center gap-2 rounded-full border border-cyan-700/40 bg-cyan-950/30 px-3 py-1 text-xs font-semibold uppercase tracking-[0.12em] text-cyan-200">
          <Activity className="h-3.5 w-3.5" /> {feed.events.length} Events
        </div>
      </header>

      {feed.events.length === 0 ? (
        <section className="rounded-xl border border-slate-800 bg-slate-950/70 p-4">
          <h3 className="text-sm font-semibold tracking-tight text-slate-100">{feed.emptyState.title}</h3>
          <p className="mt-2 text-sm leading-relaxed text-slate-400">{feed.emptyState.description}</p>
        </section>
      ) : (
