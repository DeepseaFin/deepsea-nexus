...
import type { ApprovalNextAction, ApprovalPanelConfig } from "@/lib/customer/approval/approval-panel.types";

function priorityTone(priority: ApprovalNextAction["priority"]): string {
  if (priority === "high") {
...
export interface ApprovalDecisionCardProps {
  ...
  readonly action: ApprovalNextAction;
}

export default function ApprovalDecisionCard({ config, action }: ApprovalDecisionCardProps) {
  return (
