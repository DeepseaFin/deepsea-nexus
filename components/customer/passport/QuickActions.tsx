...
export default function PassportQuickActions({ actions }: PassportQuickActionsProps) {
  return (
    <UICard variant="subtle" className="p-5 sm:p-6">
      ...
      <div className="mt-4 grid gap-2 sm:grid-cols-2">
        {actions.map((action) => (
          <UIButton key={action.id} variant={action.variant ?? "ghost"} fullWidth className="justify-start">
            {action.label}
          </UIButton>
        ))}
      </div>
