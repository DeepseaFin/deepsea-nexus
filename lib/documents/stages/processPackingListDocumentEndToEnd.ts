...
import { createPackingListProcessingStage } from "@/lib/documents/stages/PackingListProcessingStage";

export interface PackingListEndToEndDependencies {
...
export interface PackingListEndToEndInput {
  readonly documentId: string;
  readonly customerId?: string;
  readonly passportId?: PassportId | string;
}

export async function processPackingListDocumentEndToEnd(
  input: PackingListEndToEndInput,
  dependencies: PackingListEndToEndDependencies,
): Promise<DocumentIntelligencePipelineResult> {
  const evidenceExtractionStage = createPackingListProcessingStage();
