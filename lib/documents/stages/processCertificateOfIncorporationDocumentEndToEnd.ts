...
import { createCertificateOfIncorporationProcessingStage } from "@/lib/documents/stages/CertificateOfIncorporationProcessingStage";

export interface CertificateOfIncorporationEndToEndDependencies {
...
export interface CertificateOfIncorporationEndToEndInput {
  readonly documentId: string;
  readonly customerId?: string;
  readonly passportId?: PassportId | string;
}

export async function processCertificateOfIncorporationDocumentEndToEnd(
  input: CertificateOfIncorporationEndToEndInput,
  dependencies: CertificateOfIncorporationEndToEndDependencies,
): Promise<DocumentIntelligencePipelineResult> {
  const evidenceExtractionStage = createCertificateOfIncorporationProcessingStage();
