...
import { createInvoiceProcessingStage } from "@/lib/documents/stages/InvoiceProcessingStage";

export interface InvoiceEndToEndDependencies {
...
export interface InvoiceEndToEndInput {
  readonly documentId: string;
  readonly customerId?: string;
  readonly passportId?: PassportId | string;
}

export async function processInvoiceDocumentEndToEnd(
  input: InvoiceEndToEndInput,
  dependencies: InvoiceEndToEndDependencies,
): Promise<DocumentIntelligencePipelineResult> {
  const evidenceExtractionStage = createInvoiceProcessingStage();
