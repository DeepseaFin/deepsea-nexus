...
import { buildRelationshipDataset, money, type EntityNode } from '@/components/atlas/relationship-intelligence/data';
...
export default function RelationshipIntelligencePage() {
  ...
  const [status, setStatus] = useState('ATLAS network intelligence is synchronized.');
  const [spotlightOpen, setSpotlightOpen] = useState(false);

  const dataset = useMemo(() => buildRelationshipDataset(1100), []);
  ...
  const executeTopAction = (label: string) => {
    if (label === 'Refresh') {
      setStatus('Relationship intelligence refreshed from institutional graph index.');
      return;
    }
      ...
      <div className="mx-auto max-w-[1920px] space-y-4 pb-28">
        <SectionCard title="Relationship Intelligence" icon={BrainCircuit}>
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              <p className="text-sm text-slate-300">Enterprise Network Intelligence &amp; Exposure Analysis</p>
        ...
        {tab === 'Ownership' && (
          <SectionCard title="Ownership Intelligence Tree" icon={Building2}>
            <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
              <div className="rounded-lg border border-slate-800 bg-slate-950/70 p-3 text-sm text-slate-200">Ultimate Beneficial Owner: Nexus Capital Trust (58%)</div>
              <div className="rounded-lg border border-slate-800 bg-slate-950/70 p-3 text-sm text-slate-200">Shareholders: 7 active across 4 jurisdictions</div>
              <div className="rounded-lg border border-slate-800 bg-slate-950/70 p-3 text-sm text-slate-200">Directors: 12 mapped with board interlocks</div>
              <div className="rounded-lg border border-slate-800 bg-slate-950/70 p-3 text-sm text-slate-200">Parent: Deepsea Financial Holdings</div>
              <div className="rounded-lg border border-slate-800 bg-slate-950/70 p-3 text-sm text-slate-200">Subsidiaries: 14 linked operating entities</div>
              <div className="rounded-lg border border-slate-800 bg-slate-950/70 p-3 text-sm text-slate-200">Board Members: 19 with cross-entity roles</div>
            </div>
          </SectionCard>
        )}
        ...
        {tab === 'Banking' && (
          <SectionCard title="Banking Relationship Intelligence" icon={Landmark}>
            <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
              ...
              <div className="rounded-lg border border-slate-800 bg-slate-950/70 p-3 text-sm text-slate-200">Collection Banks: 11</div>
              <div className="rounded-lg border border-slate-800 bg-slate-950/70 p-3 text-sm text-slate-200">SWIFT Mappings: 27 verified</div>
              <div className="rounded-lg border border-slate-800 bg-slate-950/70 p-3 text-sm text-slate-200">IBAN Relationships: 162 active</div>
              <div className="rounded-lg border border-slate-800 bg-slate-950/70 p-3 text-sm text-slate-200">Limits: AED 420M aggregate</div>
              ...
              <div className="rounded-lg border border-slate-800 bg-slate-950/70 p-3 text-sm text-slate-200">Bank Relationship Health: Stable</div>
              <div className="rounded-lg border border-slate-800 bg-slate-950/70 p-3 text-sm text-slate-200">Concentration Signal: Moderate</div>
            </div>
          </SectionCard>
        )}
          ...
          <SectionCard title="Legal Intelligence" icon={Gavel}>
            <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
              {['Linked Documents', 'Guarantees', 'Assignments', 'Security', 'Power of Attorney', 'Court Cases', 'Disputes', 'Legal Opinions'].map((item) => (
                <div key={item} className="rounded-lg border border-slate-800 bg-slate-950/70 p-3 text-sm text-slate-200">{item}: mapped and active</div>
              ))}
            </div>
          </SectionCard>
        )}
